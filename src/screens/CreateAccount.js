...
import {View, SafeAreaView, TouchableOpacity, StyleSheet, Image, Text} from 'react-native';
...
const CreateAccount = ({navigation}) => {
  ...
  const [competitionLevel, setCompetition] = useState('');
  ...
  const isButtonDisabled = !!!firstName || !!!newSports.length || !!!(competitionLevel && competitionLevel !== '0');
  const onSaveClick = () => {
    try {
      let obj = {};
      if (!!newSports.length) obj.sports = newSports.join(', ');
      if (!!isImageChanged) {
        dispatch(updateImage({image: newImage, token}));
      }
      if (!!firstName) obj.firstName = firstName;
      if (!!lastName) obj.lastName = lastName;
      if (!!competitionLevel) obj.competition = competitionLevel;
      if (!!!isButtonDisabled) dispatch(updateUser({...obj, token}));
      ...
      <SafeAreaView>
        ...
        <View style={styles.pageContainer}>
          <ScrollView>
            ...
            <AppButton container={{marginBottom: 20}} disabled={isButtonDisabled} label="next" onPress={onSaveClick} />
          </ScrollView>
        </View>
      </SafeAreaView>
      <AppSheet
